...
export function isConversationLoggingEnabled(): boolean {
  return !!(
    process.env[AGY_LOG_FILE_ENV_VAR] || process.env[AGY_LOG_DIR_ENV_VAR]
  );
}
 ...
 */
function resolveLogTarget(timestamp: string): string | null {
...
function formatEntry(e: ConversationLogEntry): string {
...
export function appendConversationLog(entry: ConversationLogEntry): void {
  try {
    ...
    if (!target) return; // logging disabled
